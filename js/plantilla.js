define(function (require) {
    return [
        {//Sistema Reproductor
            ...
            'juego_f' : '<div id="fondo"><h1 id="insturccion">Arrastra los nombres a su lugar correspondiente</h1><div id="etiquetas"></div>{{#botones}}<p class="nombre">{{boton}}</p>{{/botones}}<div id="grafica">{{#respuestas}}<div class="respuesta" id="{{res}}"></div>{{/respuestas}}<img src="{{grafica}}"></div><div id="botones"><button id="btn_reg_f">Atrás</button><button id="btn_rev">Comprobar</button><button id="btn_rin_f">Reiniciar</button></div><div id="triangulo_f"></div><div id="Modal" class="modal"><div class="modal-content"><img src=""><button id="Close" class="close">Aceptar</button></div></div></div>',
            
            'juego_m' : '<div id="fondo"><h1 id="insturccion">Arrastra los nombres a su lugar correspondiente</h1><div id="etiquetas"></div>{{#botones}}<p class="nombre">{{boton}}</p>{{/botones}}<div id="grafica">{{#respuestas}}<div class="respuesta" id="{{res}}"></div>{{/respuestas}}<img src="{{grafica}}"></div><div id="botones"><button id="btn_reg_m">Atrás</button><button id="btn_rev">Comprobar</button><button id="btn_rin_m">Reiniciar</button></div><div id="triangulo_m"></div><div id="triangulo_mf"></div><div id="Modal" class="modal"><div class="modal-content"><img src=""><button id="Close" class="close">Aceptar</button></div></div></div>'
        }
    ];
});
